...
# Usage: GetPrecisionBootOrder.py [options]
#
# Options:
...
# UseCases:
# GetPrecisionBootOrder.py -i <IP Address> -u <Username> -p <Password>
#
# NOTE: This script will only work with IMC version 2.0 and higher.
...
if __name__ == "__main__":
    try:
        ...
        bootDevices = get_child(handle,in_dn="sys/rack-unit-1/boot-precision")
        #write_object(bootDevices)
        
        bootOrderDict = {}
        for bootDevice in bootDevices:
            bootOrderDict[int(bootDevice.get_attr("Order"))] = \
            bootDevice
        ...
        for bootOrder in sorted(bootOrderDict.keys()):
            print str(bootOrder) +") "+ \
            bootOrderDict[bootOrder].get_attr("Rn") + \
